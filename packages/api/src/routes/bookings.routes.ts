...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const booking = await BookingService.getBooking(req.params['id']!, req.user!.sub, req.user!.role);
    res.json({ success: true, data: booking });
  } catch (err) { next(err); }
});

// POST /bookings/:id/accept  (CLEANER)
router.post('/:id/accept', requireRole('CLEANER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/decline', requireRole('CLEANER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/start', requireRole('CLEANER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/complete', requireRole('CLEANER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/confirm', requireRole('CLIENT'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.patch('/:id/cancel', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { reason } = z.object({ reason: z.string().optional() }).parse(req.body);
    const booking = await BookingService.cancelBooking(req.params['id']!, req.user!.sub, req.user!.role, reason);
    res.json({ success: true, data: booking });
  } catch (err) { next(err); }
});

// POST /bookings/:id/dispute  (CLIENT)
router.post('/:id/dispute', requireRole('CLIENT'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const body = z.object({
...
router.post('/:id/bids', requireRole('CLEANER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const body = z.object({
...
router.get('/:id/bids', requireRole('CLIENT'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/bids/:bidId/accept', requireRole('CLIENT'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const booking = await BookingService.acceptBid(req.params['id']!, req.params['bidId']!, req.user!.sub);
    res.json({ success: true, data: booking });
  } catch (err) { next(err); }
});
